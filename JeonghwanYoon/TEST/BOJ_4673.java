...
public class BOJ_4673 {
	...
	public static void main(String[] args) {
		 ...
		 * 양의 정수 n에 대해서 d(n)을 n과 n의 각 자리수를
		...
		//예상 풀이 : 나머지연산으로 자릿수를 배열에 넣어서 셀프넘버를 찾아서 출력하면 되지않을까?
		int arr[] = new int[10001]; // n은 10000까지이므로
		
		// 1. 각 자리수를 더한 함수 구하기
		// 2. 위 함수에서 리턴 된값 뺀 나머지 출력(셀프넘버)
		
		for(int i=1; i<arr.length; i++) // 각 자리수를 더한 함수를 arr배열에 입력
		{
			int n = sum(i);
		...
		for(int i=1; i<arr.length; i++) // 각 자리수를 더한 함수를 제외한 셀프넘버 출력
		{
			if(arr[i] == 0) 
