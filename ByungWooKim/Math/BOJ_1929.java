 ...
 *
 * # 배운점
 * 1. 난 int배열로 소수가 아니라면 0으로 체크해주고 맞다면 그 소수값 그대로를 넣어줬는데
 *    boolean 배열로 인덱스가 소수인 값은 false로 소수가 아닌값은 true로 해서도 푸는 방법이 있었다.
 *
 * 2. 출력결과가 무수히 많이 나올수 있는 것들은 결과값이 나올떄마다 출력하는 것보다
 *    StringBuilder에 결과값들을 모두 넣어준 다음에 그 값들을 한번에 출력하는것에는 성능차이가 어마무시했다..
 *    StringBuilder를 쓰니 메모리가 20664KB에 184ms로 이전의 366480KB에 982ms보다 어마어마하게 성능이 향상되었다.
 *
 */
