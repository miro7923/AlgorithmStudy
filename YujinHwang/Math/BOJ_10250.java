...
/*
 * # 풀이
 * N을 H로 나눈 몫 + 1이 방 번호의 맨 뒷자리가 되고 N을 H로 나눈 나머지만큼 앞자리 층수를 올려주면 되기 때문에
 * 정답을 구하기 위한 계산식은 (N % H) * 100 + (N / H + 1) * 100 이 된다.
 * 
...
public class Main {
	...
	public static void main(String[] args) throws IOException {
		...
		// N을 H로 나눈 몫 + 1이 방번호의 맨 뒷자리가 되고 
		// 나머지만큼 앞자리 층수를 올려주면 됨
		
		int T = Integer.parseInt(bf.readLine()); // 테스트케이스
		int H, W, N, ans = 0;
		int lastNum, rem; // 방번호의 맨 뒷자리, 손님 번호를 층수로 나누고 남은 나머지
		
		for (int testCase = 0; T > testCase; testCase++)
		{
			StringTokenizer st = new StringTokenizer(bf.readLine());
			H = Integer.parseInt(st.nextToken()); // 호텔의 층 수 - 세로길이
			W = Integer.parseInt(st.nextToken()); // 각 층의 방 수 - 가로길이
			N = Integer.parseInt(st.nextToken()); // 몇 번째 손님인지
			
			// N이 H의 배수인 경우 - 손님은 꼭대기 층에 가야 함
			if (0 == N % H)
			{
				lastNum = N / H;
				ans = H * 100 + lastNum; // 층수는 항상 꼭대기인 H로 고정되기 때문에 1번에서 몇 번째 떨어진 방 인지만 구하면 된다.
			}
			else // 배수가 아닐 때엔 아래층에 있음
			{
				lastNum = N / H + 1;
				rem = N % H;
				ans = rem * 100 + lastNum; // 나머지만큼 층수를 올려주고 1번에서 떨어진 거리를 더해준다.
			}
